      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900 tracking-tight italic">SmartCampus<span className="text-indigo-600 font-medium tracking-normal text-base ml-1 border-l border-slate-300 pl-2 not-italic">Hub</span></h1>
            </div>
          </div>

          <nav className="hidden md:flex items-center gap-1 bg-slate-100/50 p-1 rounded-2xl">
            {[{ label: 'Home', path: '/home' }, { label: 'Tickets', path: '/student-tickets' }].map(item => (
              <button key={item.label} onClick={() => navigate(item.path)} className="px-5 py-2 text-sm font-bold text-slate-600 hover:text-indigo-600 hover:bg-white rounded-xl transition-all">
                {item.label}
              </button>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4">
            <NotificationDropdown userId={user?.id} isOpen={showNotifications} onClose={() => setShowNotifications(false)} onToggle={() => setShowNotifications((c) => !c)} />
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="w-10 h-10 rounded-2xl bg-indigo-600 text-white font-black text-sm shadow-lg shadow-indigo-200 flex items-center justify-center hover:scale-105 transition-transform">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-[0.2em]">Student Services</span>
          </div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight mb-4">
            {activeTab === 'book' ? 'Reserve Campus Spaces' : 'My Reservation History'}
          </h2>
          ...
          <div className="flex p-1.5 bg-slate-200/50 rounded-2xl w-fit">
            <button
              ...
              className={`flex items-center gap-2 px-8 py-3 rounded-xl text-sm font-black transition-all ${activeTab === 'book' ? 'bg-white text-indigo-600 shadow-xl shadow-indigo-500/10' : 'text-slate-500 hover:text-slate-700'}`}
            >
              <LayoutGrid className="w-4 h-4" /> Book a Resource
            </button>
            <button
              onClick={() => { setActiveTab('my-bookings'); setSearchTerm(''); }}
              className={`flex items-center gap-2 px-8 py-3 rounded-xl text-sm font-black transition-all ${activeTab === 'my-bookings' ? 'bg-white text-indigo-600 shadow-xl shadow-indigo-500/10' : 'text-slate-500 hover:text-slate-700'}`}
            >
        ...
        {/* ── Filters Bar ── */}
        <div className="bg-white p-4 rounded-[2rem] border border-slate-200 shadow-xl shadow-slate-200/40 mb-10 flex flex-col lg:flex-row gap-4">
          <div className="relative flex-1 group">
            ...
            <input
              ...
              className="w-full pl-12 pr-6 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-200 transition-all text-slate-700 font-medium"
            />
          </div>
          
          <div className="flex gap-4">
            {activeTab === 'book' ? (
              <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="pl-6 pr-10 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-600 text-sm">
                <option value="All">All Types</option>
                {Object.keys(TYPE_META).map(t => <option key={t} value={t}>{TYPE_META[t].label}</option>)}
              </select>
            ) : (
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="pl-6 pr-10 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-600 text-sm">
                {['All', 'PENDING', 'APPROVED', 'REJECTED', 'CANCELLED'].map(s => <option key={s} value={s}>{s === 'All' ? 'All Status' : s}</option>)}
              </select>
            )}
            <button onClick={fetchData} className="p-4 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-colors text-slate-400 hover:text-indigo-600">
