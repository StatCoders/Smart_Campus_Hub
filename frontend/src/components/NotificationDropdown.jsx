...
import { CheckCheck, AlertCircle, Inbox, Settings, Filter, ChevronDown } from 'lucide-react';
...
export default function NotificationDropdown({ userId, isOpen, onClose, onToggle }) {
  ...
  const handleNotificationClick = async (notification) => {
    ...
    if (referenceType === 'BOOKING' && referenceId) {
      navigate(isStudent ? `/student-bookings?highlight=${referenceId}` : `/bookings?highlight=${referenceId}`);
    } else if (referenceType === 'TICKET' && referenceId) {
      navigate(isStudent ? `/student-tickets?highlight=${referenceId}` : `/tickets?highlight=${referenceId}`);
    } else {
      if (type === 'BOOKING') {
    ...
    <div ref={dropdownRef} className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 top-full z-50 mt-3 w-[24rem] overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.16)] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <p className="text-sm font-bold text-slate-950 uppercase tracking-tight">Notifications</p>
                  <button 
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-xl border px-2.5 py-1.5 text-[11px] font-bold transition-all duration-200 ${
                      priorityFilter !== 'ALL' 
                  ...
                  >
                    ...
                    <span>{priorityFilter === 'ALL' ? 'All' : priorityFilter}</span>
                    <ChevronDown className={`h-3 w-3 transition-transform ${isFilterOpen ? 'rotate-180' : ''}`} />
                  </button>

                  {isFilterOpen && (
                    <div className="absolute right-0 top-full z-[60] mt-2 w-40 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-[0_10px_40px_rgba(0,0,0,0.12)] ring-1 ring-slate-200/50 animate-in fade-in slide-in-from-top-2 duration-200">
                      <p className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-400">Filter By</p>
                      {['ALL', 'HIGH', 'MEDIUM', 'LOW'].map((p) => {
                        let label = p === 'ALL' ? 'All Notifications' : `${p} Priority`;
                        if (p === 'HIGH') label = 'Tickets (High)';
                        if (p === 'MEDIUM') label = 'Bookings (Medium)';
                        if (p === 'LOW') label = 'System (Low)';
                        
                        return (
                          <button
                            key={p}
                            onClick={() => { setPriorityFilter(p); setIsFilterOpen(false); }}
                            className={`w-full rounded-xl px-3 py-2.5 text-left text-[11px] font-bold transition-all duration-200 ${
                              priorityFilter === p 
          ...
          {/* Content */}
          <div className="max-h-[30rem] overflow-y-auto bg-white p-2 custom-scrollbar">
            {loading && (
                    ...
                    >
                      <div className="flex items-start gap-4">
                        ...
                        <div className={`mt-0.5 h-2 w-2 flex-shrink-0 rounded-full ${
                          notification.priority === 'HIGH' ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.4)] animate-pulse' :
